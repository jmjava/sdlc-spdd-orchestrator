...
Your job is to review everything sitting in the gitignored runtime (`sdlc-spdd/.sdlc/`),
...
1. Determine the active Work ID from `sdlc-spdd/.sdlc/sessions/current-session.md` or
   from the request. If reviewing all staged work, say so explicitly.
2. Inventory the gitignored runtime state:
   - `./sdlc-spdd/scripts/sdlc.sh accept --list` — staged record ids, kinds, titles
   - `sdlc-spdd/.sdlc/staged/lessons.jsonl` — full staged bodies
   - `sdlc-spdd/.sdlc/sessions/current-session.md` — hot brief (context only, never promoted)
3. Review every staged record for consistency:
   ...
   - the Work ID has a canvas (`sdlc-spdd/spdd/canvas/<WORK-ID>.md`) or requirement
...
4. Review every staged record for coherence and value:
   ...
   - merge near-duplicates by re-staging one corrected record via
     `./sdlc-spdd/scripts/sdlc.sh capture` before promoting
5. Promote and clean the stage:
   - all good: `./sdlc-spdd/scripts/sdlc.sh accept --work-id <ID>`
   - keep some: `./sdlc-spdd/scripts/sdlc.sh accept --ids <a,b,c> --discard-rest`
   - never edit `sdlc-spdd/spdd/memory/lessons.jsonl` by hand
6. Git-stage what the promotion changed so the batch commit (or amend) picks
   it up: `git add sdlc-spdd/spdd/memory/lessons.jsonl sdlc-spdd/spdd/memory/registry.jsonl` plus
   any contract files this Work ID touched (`sdlc-spdd/spdd/canvas/`, `sdlc-spdd/spdd/reviews/`,
   `sdlc-spdd/spdd/sync/`, `sdlc-spdd/requirements/`).
...
- Records promoted to `sdlc-spdd/spdd/memory/lessons.jsonl`
