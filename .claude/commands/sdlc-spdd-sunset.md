---
description: Close out a Work ID by syncing GitHub PR, GitHub issue, commit, and Jira state into the lesson ledger via the Python engine.
...
Your job is to close out a Work ID by collecting GitHub PR, GitHub issue, commit, and Jira state through the Python engine (`./sdlc-spdd/scripts/sdlc.sh sunset`) and staging that snapshot into the lesson ledger. Do not implement code. Do not archive artifacts unless the user explicitly asks.

## Required Behavior


1. Parse an optional Work ID. Do not invent Work IDs.
2. If Work ID is omitted, try the active pointer via `./sdlc-spdd/scripts/sdlc.sh next` (or `./scripts/sdlc.sh next` in the orchestrator repo) or `sdlc-spdd/.sdlc/sessions/current-session.md`. If still unknown, stop and ask for a Work ID.
