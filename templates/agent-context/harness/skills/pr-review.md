...
1. Run `./sdlc-spdd/scripts/start-agent-session.sh --target . --work-id <WORK-ID> --phase review`.
...
8. Persist review memory with `./sdlc-spdd/scripts/capture-session-memory.sh --target . --work-id <WORK-ID> --phase review --summary "<review summary>" --validation "<checks>" --next "<next command>"`.
