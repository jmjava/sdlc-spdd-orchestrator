...
2. Start or resume context with `./sdlc-spdd/scripts/start-agent-session.sh --target . --work-id <WORK-ID> --phase plan`.
...
6. Before each new agent session, run `./sdlc-spdd/scripts/resync-agent-session.sh --target . --work-id <WORK-ID> --check-only`.
...
11. Persist memory with `./sdlc-spdd/scripts/capture-session-memory.sh --target . --work-id <WORK-ID> --phase <phase> --summary "<summary>" --validation "<tests>" --next "<next command>"`.
