...
6. If no active Work ID, suggest `/sdlc-claim <WORK-ID>` or `./sdlc-spdd/scripts/sdlc.sh resume <WORK-ID>`.
...
- Registry events live in `sdlc-spdd/spdd/memory/registry.jsonl`; lessons are accepted at retro/sync with `./sdlc-spdd/scripts/sdlc.sh accept --work-id <ID>`
