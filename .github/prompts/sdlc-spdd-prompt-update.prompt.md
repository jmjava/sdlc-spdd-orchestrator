...
9. Stage a decision record explaining the intent change via `./sdlc-spdd/scripts/sdlc.sh capture --decision ...` (never edit `sdlc-spdd/spdd/memory/lessons.jsonl` by hand).
...
- Staged decision record in `sdlc-spdd/.sdlc/staged/lessons.jsonl` (promoted at retro/sync with `./sdlc-spdd/scripts/sdlc.sh accept --work-id <ID>`)
