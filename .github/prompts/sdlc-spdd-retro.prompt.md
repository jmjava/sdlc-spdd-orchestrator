...
1. Gate first: run `./sdlc-spdd/scripts/sdlc.sh gate retro --work-id <WORK-ID>` (in the
...
3. Read the review report (`sdlc-spdd/spdd/reviews/<WORK-ID>-review.md`).
...
9. Stage decision/pitfall/pattern records via `./sdlc-spdd/scripts/sdlc.sh capture` flags (never edit `sdlc-spdd/spdd/memory/lessons.jsonl` by hand).
10. Promote accepted records with `./sdlc-spdd/scripts/sdlc.sh accept --work-id <ID>`.
...
- Staged records in `sdlc-spdd/.sdlc/staged/lessons.jsonl`
- Accepted records promoted to `sdlc-spdd/spdd/memory/lessons.jsonl` via `./sdlc-spdd/scripts/sdlc.sh accept --work-id <ID>`
