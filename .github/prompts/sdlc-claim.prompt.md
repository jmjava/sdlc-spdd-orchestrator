...
1. Require a Work ID argument (e.g. `FEAT-001-shared-script-library`). If missing, ask for it or run `./sdlc-spdd/scripts/sdlc.sh list-work` (or `./scripts/sdlc.sh list-work` in the orchestrator repo).
2. Run `./sdlc-spdd/scripts/sdlc.sh claim <WORK-ID>` (or `./scripts/sdlc.sh claim <WORK-ID>`) with optional flags the user supplied: `--force`, `--phase`, `--branch`, `--pr`, `--jira`, `--note`.
...
4. After a successful claim, run `./sdlc-spdd/scripts/sdlc.sh next` (or `./scripts/sdlc.sh next`) to show phase and the recommended next command.
