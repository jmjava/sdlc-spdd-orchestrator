...
mkdir -p "${WORK}/sdlc-spdd/harness/skills"

cat > "${WORK}/sdlc-spdd/harness/skills/team-norms.md" <<'EOF'
---
skill: team-norms
aliases: _
phases: *
...
cat > "${WORK}/sdlc-spdd/harness/skills/coding-style.md" <<'EOF'
---
skill: coding-style
aliases: _
...
cat > "${WORK}/sdlc-spdd/harness/skills/TDD.md" <<'EOF'
---
skill: TDD
aliases: _
phases: code
...
cp "${REPO_ROOT}/templates/agent-context/harness/phase-index.md" \
  "${WORK}/sdlc-spdd/harness/phase-index.md"
cp "${REPO_ROOT}/templates/agent-context/harness/quality-gates.md" "${WORK}/sdlc-spdd/harness/" 2>/dev/null || \
  printf '# Quality Gates\n\n' > "${WORK}/sdlc-spdd/harness/quality-gates.md"
# Tab-delimited skill metadata requires a non-empty aliases field when phases are present.
for skill in bugfix pr-review; do
  if ! grep -q '^aliases:' "${WORK}/sdlc-spdd/harness/skills/${skill}.md"; then
    sed -i "/^skill: ${skill}/a aliases: ${skill}" "${WORK}/sdlc-spdd/harness/skills/${skill}.md"
  fi
done
...
mkdir -p "${WORK}/sdlc-spdd/harness/skills"
cat > "${WORK}/sdlc-spdd/harness/skills/review-checklist.md" <<'EOF'
---
...
if grep -Fq "## Resolved Context" "${WORK}/sdlc-spdd/.sdlc/sessions/current-session.md" && \
   grep -Fq "team-norms.md" "${WORK}/sdlc-spdd/.sdlc/sessions/current-session.md"; then
...
mkdir -p "${WORK}/sdlc-spdd/spdd/analysis" "${WORK}/sdlc-spdd/spdd/canvas"
cat > "${WORK}/sdlc-spdd/spdd/analysis/FEAT-050-billing-analysis.md" <<'AN'
...
- src/billing
AN
cat > "${WORK}/sdlc-spdd/spdd/canvas/FEAT-050-billing.md" <<'CV'
# Canvas
CV
...
mkdir -p "${WORK}/sdlc-spdd/spdd/tasks"
echo "# API tasks" > "${WORK}/sdlc-spdd/spdd/tasks/FEAT-050-billing-api-test.md"
...
if grep -Fq "Also read @spdd/canvas/FEAT-050-billing.md" "${WORK}/sdlc-spdd/.sdlc/sessions/current-session.md"; then
...
mkdir -p "${WORK}/sdlc-spdd/spdd/memory" "${WORK}/sdlc-spdd/.sdlc/staged"
printf '%s\n' \
  ...
  > "${WORK}/sdlc-spdd/spdd/memory/lessons.jsonl"
out="$("${RESOLVE}" --target "${WORK}" --phase code --work-id FEAT-050-billing --format paths)"
assert_contains "${out}" ".sdlc/resolved/progress-FEAT-050-billing.md" "scoped progress excerpt"
excerpt="${WORK}/sdlc-spdd/.sdlc/resolved/progress-FEAT-050-billing.md"
if [[ -f "${excerpt}" ]]; then
