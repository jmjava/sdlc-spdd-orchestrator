...
is required, then that it happens automatically, including when a file is
mislabeled as current schema but still has the old columns.
"""
...
_V5_LESSON_COLUMNS = {"title", "phase", "keywords", "staged"}
...
def test_v5_persist_row_fails_on_old_schema_without_upgrade(tmp_path: Path) -> None:
    ...
    assert not _V5_LESSON_COLUMNS <= cols
    conn.close()
...
def test_persist_upgrades_v4_sqlite_in_place_then_succeeds(tmp_path: Path) -> None:
    ...
    assert _V5_LESSON_COLUMNS <= col_names
...
def test_persist_upgrades_stale_v5_label_with_v4_columns(tmp_path: Path) -> None:
    """A file labeled current schema but missing v5 columns must still upgrade first."""
    ...
    result = store.persist_lesson(
        ...
        work_id=wid,
        area="engine",
        body="stale label still upgrades",
        source="stale-label-check",
        project_guide=False,
    )
    assert result.sqlite.get("ok") is True, result.sqlite
    assert result.sqlite.get("schema") == SCHEMA_VERSION
    ...
    with idx.connect() as conn:
        cols = {row[1] for row in conn.execute("PRAGMA table_info(lessons)").fetchall()}
    assert _V5_LESSON_COLUMNS <= cols
    rows = idx.lessons_for_work(wid)
    assert any("stale label still upgrades" in (r.get("body") or "") for r in rows)
