...
def is_playground(target: Path | str) -> bool:
    return (Project(Path(target).expanduser().resolve()).sdlc_dir / MARKER_NAME).is_file()
...
def materialize_playground(
    ...
    *,
    ...
    memory = project.spdd_dir / "memory"
    canvas_dir = project.spdd_dir / "canvas"
    analysis_dir = project.spdd_dir / "analysis"
    reviews_dir = project.spdd_dir / "reviews"
    milestones = project.requirements_dir / "milestones"
    adf_dir = root / "adf"  # ADF viewer store lives at the project root
    ...
    fake_home = seed_fake_guide_tree(root)
    save_json_dict(sdlc / "playground-runtime.json", default_runtime(root))
    save_json_dict(
        sdlc / "persistence-config.json",
        {
            "backends": ["git-pointers", "sqlite"],
            "guide_url": "",
            "notes": "Playground: git + sqlite. Guide/Jira/GitHub are in-process fakes.",
        },
    )
    save_json_dict(
        ...
        {
            "tracker": "github",
            "notes": "Playground tokens are fake — sync never leaves this process.",
            "jira": {
            ...
            "github": {
                "repo": "example/console-playground",
                "token": FAKE_GH_TOKEN,
            },
        },
    )
    save_json_dict(
        sdlc / "guide-config.json",
        {
            ...
            "host": "127.0.0.1",
            "port": 21337,
            "notes": "Playground fake Guide — Start/Stop toggle in-process state, no JVM/Docker.",
        },
    )
    ...
    (root / "PLAYGROUND.md").write_text(
        ...
        "Do not install this into a real app. Guide/Jira/GitHub are in-process fakes.\n",
        encoding="utf-8",
    )
    return root
...
def _write_requirement(
    ...
    *,
    ...
    gh_url = (
        f"https://github.com/example/console-playground/issues/{github_number}"
        if github_number
        else ""
    )
    path.write_text(
...
Playground requirement. Sync uses in-process Jira/GitHub fakes.

## GitHub

- Number: {github_number or "TBD"}
- Title: {summary}
- URL: {gh_url}
""",
        encoding="utf-8",
