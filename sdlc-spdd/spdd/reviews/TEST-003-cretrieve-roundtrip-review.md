...
T01 adds the hermetic C-RETRIEVE suite for default CI: persist/accept one pitfall, then require the same id from the git ledger, SQLite parity, and mocked Guide parity. Live Guide+Neo4j round-trips already exist (`test_guide_projection_roundtrip.py` / `test-guide-stack-experimental`). Unreachable Guide is skip, not a pass. This review's **scope removed** drift and usefulness. `embabel-dif` is out of this review.
...
2. **Safeguard:** This review's scope **removed** drift and usefulness. `embabel-dif` is out of this review. Those are not leftover TEST-003 work.
