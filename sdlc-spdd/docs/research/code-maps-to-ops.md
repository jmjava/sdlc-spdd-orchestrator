...
`/sdlc-spdd-review` runs `scripts/check-operation-diff-scope.sh` (pure helper: `check_operation_diff_scope` in `engine/src/sdlc_engine/canvas.py`). Changed paths from `git diff --name-only HEAD` (working tree vs HEAD) and optional `git diff --name-only <base>...HEAD` must be a subset of the selected/completed T## `Files:` union (or all T## if none selected) plus allowed test paths:
