...
| **C-RETRIEVE** | A stored lesson can be found again in **three storage modes**, and **retrieve context** (work / area / kind / query) returns that subset rather than sibling records | Ledger: persist/accept then `context retrieve` returns the id. **T04:** given two Work IDs and three areas, retrieve-by-work / area / kind / query returns the matching ids and excludes the rest. SQLite: `context parity` plus `lessons_for_work` / `lessons_for_area` match those sets. Guide/Neo4j: live `work_subgraph` is per-work; `area_lessons` is cross-work for one area. | **TEST-003** hermetic ledger + SQLite + mocked Guide **client** (smoke + context-select). **Required** live graph: `test_guide_projection_roundtrip.py` + `test_context_store_guide_live.py` (`test_live_persist_enters_all_backends`, `test_live_context_selects_right_records`) via `test-guide-stack-live.sh`. FEAT-017 + CHORE-003 supporting. | Same; all three modes stay in-scope | Drift and usefulness were **removed from this review's scope**. Guide **embeddings** are not this bar. Mocked Guide is HTTP client parity, **not** the graph store. `db query --search` is a different CLI. Selectivity is not RQ4 usefulness. |
