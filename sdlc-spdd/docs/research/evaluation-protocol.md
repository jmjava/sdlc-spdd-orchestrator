...
**Method (SUT after REF-001):** Planning artifacts + REASONS Canvas + phase commands/gates + ledger retrieval, as implemented by Python `WorkflowEngine.gate_check` (the only engine; `sdlc.sh` is a dispatcher to it) plus spec-generated assistant adapters. See [engine-sut.md](engine-sut.md).

**Not the object:** CLI unit tests, adapter-text parity, or SWE-bench agent competence. Those stay in `TESTING.md`.

There is no shell gate engine. The bash workflow twins and the `SDLC_ENGINE` / `SDLC_GATE_ENGINE` switches were removed (REF-003), so every gate evaluation is Python `gate_check`; a shell fallback is not an evaluation condition because it no longer exists.
...
| **full SDLC-SPDD** | Canvas + phase commands + optional `./scripts/sdlc.sh gate --phase …` (Python `gate_check`) + `context retrieve` when the task is a follow-on | Canvas + phase artifacts | Phases + gates + retrieve |
...
| Python `gate_check` is the only gate engine (no shell fallback exists) | Network / product drift | Engine SHA |
...
| Dual engine (shell vs Python `gate_check`) | Confound for “the method” | REF-001 named Python `gate_check` as SUT; REF-003 removed the shell engine entirely |
