...
|--------|-----------------|
| Dual engine (shell `sdlc.sh` vs Python `gate_check`) | Retired (REF-001 named the SUT; REF-003 deleted the shell engine and its switches). The research SUT is Python `gate_check`; `sdlc.sh` only dispatches to it. |
...
| Gate SUT | Python `gate_check` via `sdlc.sh gate` (single engine) | [engine-sut.md](engine-sut.md); `sdlc-engine gate --phase code --work-id <WID>` |
