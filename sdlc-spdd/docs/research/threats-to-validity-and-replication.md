...
| **C-MEMORY** | FEAT-015 `context metrics --construct C-MEMORY` (C-REWORK fields on a follow-on). Retrieve `--keyword` is exact list membership; `--query` ranks title/body (**FEAT-017**). | Usefulness is C-REWORK on session 2 (TEST-001 RQ4), not retrieve-call counts. Committed dogfood ledger is empty until CHORE-003. Guide DICE embeddings are unmeasured. |
