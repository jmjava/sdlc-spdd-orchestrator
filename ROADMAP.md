...
| Guide git-incremental fork sustainment (FEAT-013): keep Layer B on `jmjava/guide`; **never** PR/merge to `embabel/guide` (pull-only sync) | Optimization (follow-on) |
...
| FEAT-013-guide-git-incremental-upstream | spdd/canvas/FEAT-013-guide-git-incremental-upstream.md | make it fast (fork-only) | In Progress — never contribute to embabel/guide |
