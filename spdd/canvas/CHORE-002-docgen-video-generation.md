...
### T06 - CI generate-all (workflow_dispatch) — **Cancelled**

- Status: Cancelled
- Description: Operator decision — no CI video render. Regenerate locally per **`TOOLING.md`**; commit MP4s manually; Pages deploy on push.

### T07 - GitHub Pages — **Complete (deploy-only CI)**

- Status: Complete
- Description: **`docs/index.html`** (3 segments) + **`.github/workflows/pages.yml`** (deploy `docs/` on push to main). No video generation in CI.
- Files: `docs/index.html`, `.github/workflows/pages.yml`
- Depends on: T04 (all three MP4s committed)
...
**Sync (2026-06-20, operator):** All three recordings composed; Pages index + deploy workflow added. T06 video-render CI **cancelled** — manual local regen + git push. T07 complete (deploy-only).
...
- PR:
- Follow-Up Tasks: `/sdlc-spdd-sync`; enable Pages in repo Settings on first deploy
