...
I1–I3 landed without promoting `LOCAL-001-kasana-harness-research`. The I3 copy-only leftover is done (#268). U2 (repo-managed `.cursor/environment.json`) is done in this repo. U3 (`sdlc-session` skill) is done in Uberorchbot (#16). Next leftover is U4 (live Automation save) in Uberorchbot — do not start it from this repo. Do not invent a DOC/FEAT.
