...
| [Engine SUT (REF-001)](../../sdlc-spdd/docs/research/engine-sut.md) | Named gate semantics | Python `WorkflowEngine.gate_check`; `SDLC_GATE_ENGINE=shell` is rejected, not an eval condition |
...
| [Kasana Agent Harness 2.0](kasana-agent-harness-2-0.md) | Product/integration **shipped** (I0–I3) | `/sdlc-spdd-code` verify-before-done; review `check-operation-diff-scope`; copy-only `pre-commit.sample`. Reject only a Python `AgentHarness` replacement |
| [Uberorchbot via SDLC-SPDD](uberorchbot-via-sdlc-spdd.md) | Product/integration **vision** (U2 done here) | **Platform = Cursor Cloud Agents + Environments.** This repo commits `.cursor/environment.json`. Automations trigger runs; Uberorchbot is plugin policy; SDLC-SPDD governs |
| [Blog: Cloud Agents as the SDLC platform](../blog/cloud-agents-as-the-sdlc-platform.md) | Publishable draft | Same architecture in blog form — not pushed to `jmjava/blogs` from this environment |
