...
|------------|------------|---------------------|--------------|
| Requirements + REASONS linkage | Stay-set files | Rows + FKs | WorkId / Canvas (+ refs) |
| Work registry / claim | Lean ledger or compact file (#84) | Claim tables | WorkId + claim model |
| Resume / session context | Pointer only on accept (#85) | Session tables | Optional; SQLite-first hot path |
...
| Retrieve assemble | Reconstruct from stay-set + pointers | Lookup/join | HTTP/MCP subgraph |
